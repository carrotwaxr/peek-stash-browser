...
## 🔒 Critical Security (Must Pass Before Release)
...
1. **Images Page User Guide** - New feature, no documentation
2. **Recommendations Feature** - Not documented
3. **Scene URLs Feature** - Not documented
4. **CHANGELOG/Release Notes** - Need to create for 3.1.0
...
**Recommended Fix:** Update `vitest.config.ts` exclude pattern:
```typescript
exclude: ["node_modules", "dist", "prisma/migrations", "**/*.integration.test.ts", "integration/**"],
```

**Stray Integration Tests:**
- `services/__tests__/StashSyncService.integration.test.ts` - Uses mocks, rename to `.stash.test.ts`
- `tests/services/SceneQueryBuilder.integration.test.ts` - True integration test, move to `integration/` folder

---

## 🎯 UI Testing (Puppeteer/Playwright) Assessment
