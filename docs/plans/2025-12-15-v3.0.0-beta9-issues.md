...
In [SearchControls.jsx:384](client/src/components/ui/SearchControls.jsx#L384):
```javascript
setSearchParams(params, { replace: true });
```

And in [urlParams.js:169](client/src/utils/urlParams.js#L169), `buildSearchParams()` creates a **brand new URLSearchParams()** that only includes filter/sort/page params - it does NOT preserve unknown params like `tab`.
...
**Files to Fix:**
- [client/src/components/ui/SearchControls.jsx](client/src/components/ui/SearchControls.jsx) - URL sync logic
- [client/src/utils/urlParams.js](client/src/utils/urlParams.js) - `buildSearchParams` function
...
The `enhanceTagsWithPerformerScenes()` function at [tags.ts:19-75](server/controllers/library/tags.ts#L19-L75) is O(n*m) where n=scenes and m=performers. It loads ALL scenes and ALL performers to calculate counts.

**Key Finding:** The real performance issue is that `enhanceTagsWithPerformerScenes()` is called at [tags.ts:261](server/controllers/library/tags.ts#L261), which is **AFTER** the filtered entity cache check (lines 151-254). This means:
...
**Files to Fix:**
- [server/controllers/library/tags.ts](server/controllers/library/tags.ts) - Move enhancement inside cache block or cache separately
...
The `NEXT_SCENE` and `PREV_SCENE` reducer actions do NOT reset `oCounter`, but `LOAD_SCENE_SUCCESS` **does** set it from the new scene's data at [scenePlayerReducer.js:108](client/src/contexts/scenePlayerReducer.js#L108):

```javascript
oCounter: action.payload.oCounter || 0,
```

The `useEffect` at [ScenePlayerContext.jsx:126-134](client/src/contexts/ScenePlayerContext.jsx#L126-L134) triggers `loadScene()` when `currentIndex` changes, which dispatches `LOAD_SCENE_SUCCESS`.
...
**Files to Fix:**
- [client/src/contexts/scenePlayerReducer.js](client/src/contexts/scenePlayerReducer.js) - Reset `oCounter` in navigation actions
