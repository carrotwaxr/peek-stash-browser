...
- A batch reads its rows' stored state first (`readStored`, one `json_each` statement; `CAST("stashUpdatedAt" AS TEXT)`, since sync stores Stash's text in a column declared DateTime and a raw read would parse it into a Date) and deletes junction rows with `DELETE ... RETURNING` (`deleteJunctionRows`), so the old far sides cost no extra read. A seed that wants to look "unchanged" must write `stashUpdatedAt` as text, as sync does; a Prisma `create` stores a number.
