...
- All three take the lock and call `runSync(mode, instanceId?)`, which runs `syncInstance(instanceId, mode, run)` for the instance given or each enabled one, then `runPostSyncSteps(run.changes, { full })` once for the whole run. `syncInstance` syncs each type in `SYNC_ORDER` (`syncEntityType` picks full, incremental or skipped for the mode), then the cleanups.
