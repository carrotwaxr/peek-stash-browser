...
- Both files take Stash's address and key from `stashInstanceManager.getCredentials(instanceId)`: the instance named, or the highest-priority enabled instance when none is. `"default"` is an ordinary id (the owner's instance has it). A named instance that is not enabled (disabled or deleted) throws `UnknownInstanceError`, which both answer with 404. The by-id handlers pass the row's `stashInstanceId`. `resolveMediaInstanceId` in `utils/mediaAccess.ts` resolves the instance for the access check through the manager's `resolveInstanceId`, so the check and the fetch look at the same instance.
