...
async function main() {
  try {
    ...
    const existingKeys = new Set(
      existingEntries.map((e) => `${e.userId}:${e.sceneId}`)
    );
    ...
    if (backup.watchHistory?.length > 0) {
      ...
      console.log(
        `Restoring ${toRestore.length} watch history entries (${backup.watchHistory.length - toRestore.length} already exist)...`
      );
      ...
      for (let i = 0; i < toRestore.length; i += chunkSize) {
        ...
        if ((i + chunkSize) % 500 === 0 || i + chunkSize >= toRestore.length) {
          console.log(
            `  Progress: ${Math.min(i + chunkSize, toRestore.length)}/${toRestore.length}`
          );
        }
      }
    }
