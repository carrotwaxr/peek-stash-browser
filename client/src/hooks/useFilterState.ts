...
export const useFilterState = ({
  ...
  const [timelinePeriod, setTimelinePeriodState] = useState<string | null>(
    null
  );
      ...
      try {
        ...
        if (hasFilterParams) {
          ...
          finalState = {
            ...
            sortField: hasUrlSort
              ? urlState.sortField
              : defaultPreset?.sort || initialSort,
            sortDirection: hasUrlDirection
              ? urlState.sortDirection
              : defaultPreset?.direction || "DESC",
            currentPage: urlState.currentPage,
            perPage: hasUrlPerPage
              ? urlState.perPage
              : defaultPreset?.perPage || urlState.perPage,
        ...
        setSortState({
          field: finalState.sortField ?? initialSort ?? "random",
          direction: finalState.sortDirection ?? "DESC",
        });
        setPaginationState({
          page: finalState.currentPage,
  ...
  const syncToUrlParams = useCallback(
    (state: FilterState, options: { replace?: boolean } = {}) => {
